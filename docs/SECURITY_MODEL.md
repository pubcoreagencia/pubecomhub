...
| **`public_suppliers`** *(View)* | - | `SELECT` | `authenticated` | `SELECT DISTINCT s.id, s.name, s.category, s.created_at FROM suppliers s JOIN master_products mp ...` | Catálogo comercial para usuários autenticados sem expor dados privados nem holding directory. |
...
| **`available_master_products`** *(View)* | - | `SELECT` | `authenticated` | Exclui `supplier_cost`. Sanitiza `metadata` JSONB. | Catálogo master comercial para Lojistas sem custos ou notas secretas de fornecedores. |
...
| **`influencer_orders`** *(View)* | - | `SELECT` | `authenticated` | Predicado: `influencer_id = auth.uid() OR affiliate_id = auth.uid() OR is_master()` | Visão restrita no banco para influencers/afiliados sem expor custos ou margem da loja. |
