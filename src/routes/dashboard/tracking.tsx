    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          <h2 className="text-2xl font-black text-white uppercase tracking-tighter italic">Tracking & Pixels</h2>
          <p className="text-[var(--hub-muted)] text-[9px] font-bold uppercase tracking-[0.3em]">Atribuição em Tempo Real · L1 - L4 Engine</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="hub-card hub-gradient-border p-8 bg-black/20">
          <h3 className="text-[11px] font-black text-white uppercase tracking-[0.3em] mb-8">Fontes de Atribuição</h3>
          <div className="space-y-8">
            {[
        ...
        <div className="lg:col-span-2 hub-card hub-gradient-border overflow-hidden bg-black/20">
          <div className="px-8 py-6 border-b border-[var(--hub-border)] flex flex-row items-center justify-between bg-black/40">
            <h3 className="text-[11px] font-black tracking-[0.3em] text-white uppercase italic">Live Event Stream</h3>
            <div className="flex items-center gap-2">
              <div className="h-1.5 w-1.5 rounded-full bg-[var(--hub-primary)] animate-pulse shadow-[0_0_8px_var(--hub-primary)]" />
              <span className="text-[9px] font-black uppercase text-[var(--hub-primary)] tracking-[0.2em]">Ao Vivo</span>
            </div>
          </div>
          <div className="divide-y divide-[var(--hub-border)]">
            {events.map((e, i) => (
              <div key={i} className="px-8 py-5 flex items-center justify-between group hover:bg-white/[0.02] transition-colors">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <p className="text-xs font-black text-white uppercase tracking-tighter italic">{e.event}</p>
                    <p className="text-[9px] font-bold text-[var(--hub-muted)] uppercase tracking-widest mt-0.5">{e.user}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-[9px] font-black text-white uppercase tracking-[0.2em]">{e.source}</p>
                  <p className="text-[9px] font-bold text-[var(--hub-muted)] mt-0.5 italic opacity-40">{e.time}</p>
                </div>
              </div>
            ))}
          </div>
