      ...
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-[var(--hub-muted)] text-[9px] font-bold uppercase tracking-[0.3em]">
              Consolidação de Resultados & Repasses
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          <div className="xl:col-span-2 space-y-4">
            <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-white px-2">
            ...
            >
              {mockFinancialSummary.top_stores.map((store, i) => (
                <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-6 py-5 text-red-400/80 italic">
                    R$ {(store.revenue * 0.45).toLocaleString("pt-BR")}
                  </td>
                  <td className="px-6 py-5 text-red-400/60 italic">
                    R$ {(store.revenue * 0.15).toLocaleString("pt-BR")}
                  </td>
                  <td className="px-6 py-5 font-black text-[var(--hub-primary)]">
                    R$ {store.profit.toLocaleString("pt-BR")}
                  </td>
          ...
          <div className="space-y-6">
            <div className="hub-card hub-gradient-border p-6 space-y-6">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div
                    key={i}
                    className="p-4 bg-black/20 border border-[var(--hub-border)] rounded-xl flex items-center justify-between"
                  >
                    <div>
                      <p className="text-[10px] font-black text-white uppercase italic">{w.user}</p>
                      <p className="text-[9px] text-[var(--hub-muted)] font-bold uppercase tracking-widest">
                        {w.date}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-black text-[var(--hub-primary)] italic">
                        {w.amount}
                      </p>
                      <button className="text-[8px] font-black text-white/40 uppercase tracking-widest hover:text-white">
                        Detalhes
                      </button>
                    </div>
                  </div>
            ...
            <div className="hub-card hub-gradient-border p-6 space-y-4 bg-[var(--hub-primary)]/5 border-[var(--hub-primary)]/20">
              ...
              <p className="text-[10px] text-[var(--hub-muted)] font-bold uppercase tracking-wider leading-relaxed">
                O próximo ciclo de repasses será encerrado em{" "}
                <span className="text-white italic">05 de Junho de 2026</span>. Certifique-se de que
                todos os pedidos entregues foram conciliados.
              </p>
