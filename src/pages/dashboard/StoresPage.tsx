      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <Button
              variant="ghost"
              className="h-9 text-[10px] font-bold uppercase tracking-widest text-[var(--hub-muted)] hover:text-white"
            >
            ...
            {stores.map((loja) => (
              <tr key={loja.id}>
                <td className="px-5 py-4">
                  <span
                    className={`px-2 py-0.5 rounded-[4px] text-[8px] font-black uppercase ${loja.status === "active" ? "bg-[var(--hub-primary)]/20 text-[var(--hub-primary)]" : "bg-slate-500/20 text-slate-400"}`}
                  >
                    {loja.status}
                  </span>
                </td>
